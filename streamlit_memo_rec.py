...
def save_and_upload_to_github(data):
    ...
    st.download_button(label="Se si volessero scaricare i dati di questa sessione, per completare l'attività successivamente o per fare richiesta al gestore dei dati di eliminarli, clicca qui:", data = StringIO(new_df.to_csv(index=False)), file_name="dati_export.csv")
